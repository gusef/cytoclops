...
ui <- navbarPage(
    ...
    tabPanel(
        title = "File loading",
        includeCSS(system.file('www', 'cytoclops.css', package =
                                   'cytoclops')),
        h4('Load raw input'),
    ...
    tabPanel(
        title = "Cleanup and QC",
        tags$div(
            id = 'ViabilityClassifier',
    ...
    tabPanel(
        title = "Display panel",
        shinyjs::useShinyjs(),
        shinyjs::extendShinyjs(script = system.file('www', 'overlay.js', package =
                                                        'cytoclops')),
        bsAlert("alert"),
        fluidRow(
            column(
                ...
                jsTreeSelectorInput(
                    'TreeGates',
                    df = data.table::rbindlist(list(list(
                        id = 'G1', text = "All"
                    ))),
                    selected = 'G1'
                )
            ...
            column(
                ...
                tags$div(id = 'MarkerSelect'),
                fluidRow(
                    column(
                        6,
                        actionButton('PolygonButton',
                                     '', 
                                     icon = icon("pencil", lib = "glyphicon")),
                        actionButton('PolygonViewButton', 
                                     '', 
                                     icon = icon("eye-open", lib = "glyphicon")),
                        actionButton('PolygonResetButton', 
                                     '', 
            ...
            column(
                ...
                shinyjs::hidden(
                    plotOutput(
                        ...
                        height = "100%",
                        click = "visne_click",
                        brush = brushOpts(id = "visne_brush")
                    )
                )
            )
        ),
    ...
    tabPanel(
        ...
        numericInput(
            inputId = 'tSNE_overlay_num_columns',
            value = 4,
            max = 8,
            min = 1,
